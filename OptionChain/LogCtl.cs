...
namespace SWI.Controls
{
    ...
    internal sealed class Logger : IDisposable
    {
        ...
        private Logger ()
        {
            ...
            try
            {
                using (RegistryKey logkey = Registry.CurrentUser.OpenSubKey ("SOFTWARE\\Muth\\IBManagePositions\\LogCtl", false))
                {
            ...
            catch (Exception)
            {
                ...
                try
                {
                    using (RegistryKey logkey = Registry.CurrentUser.CreateSubKey ("SOFTWARE\\Muth\\IBManagePositions\\LogCtl"))
                    {
                        logkey.SetValue ("LogFile", "E:\\Finance\\Logs\\IBManagePositions.log");
                        using (RegistryKey rk = logkey.CreateSubKey ("LogLevel"))
                        {
                            rk.SetValue ("", "INF");
                        }
                        logkey.SetValue ("IfTrace", 0);
                        logkey.SetValue ("TraceFile", "E:\\Finance\\Logs\\traceIBManagePositions.log");
        ...
        public void Dispose ()
        {
            lock (m_NoClients)
            {
                ...
                if ((int) m_NoClients <= 0)
                {
                    ...
                    if ((m_thr != null) && (!m_thr.Join (1000 * 20))) // twenty seconds
                    {
